...
function HowToOrderPage() {
  ...
  useEffect(() => {
    api.get('/settings/public/cms')
      .then(res => {
        if (res.data?.data?.howToOrder) {
          setContent(prev => ({
            ...prev,
            ...res.data.data.howToOrder
          }))
        }
      })
      .catch(() => {})
          ...
          {(content.steps || []).map((st, i) => (
            <div key={i} className="flex gap-6 md:gap-8 items-start relative group">
              ...
              <div className="flex-grow bg-white border border-gray-100 rounded-[24px] p-6 shadow-sm hover:shadow-md transition-all duration-300 space-y-3">
                ...
                <p className="text-[13.5px] md:text-[15px] text-gray-600 font-medium leading-relaxed text-left md:text-justify">
                  {st.desc}
                </p>
              </div>
            </div>
