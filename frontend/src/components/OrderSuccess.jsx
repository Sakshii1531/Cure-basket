...
const OrderSuccess = () => {
  ...
  const { order, shippingMethod } = location.state || {}
  ...
  const addrStr = addr
    ...
    : '—'
  const deliveryEst = shippingMethod === 'express' ? '3–7 Business Days' : '7–14 Business Days'
    ...
    <div className="bg-[#f8f9fa] min-h-screen flex items-center justify-center px-4 py-16">
      <div className="max-w-[520px] w-full text-center">
        ...
        <div className="inline-flex items-center gap-2 bg-[#E6F7F7] border border-[#006D6D]/10 px-4 py-2 rounded-full mt-4 mb-8">
          ...
          <span className="text-[13px] font-black text-[#006D6D] tracking-wider">CB-{orderId}</span>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mb-8 text-left space-y-4">
          ...
          <div className="divide-y divide-gray-50">
            {[
              { label: 'Delivery Address', value: addrStr },
              { label: 'Payment', value: 'Paid' },
              { label: 'Estimated Delivery', value: deliveryEst },
