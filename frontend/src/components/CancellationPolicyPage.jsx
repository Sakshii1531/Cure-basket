...
function CancellationPolicyPage() {
  ...
  useEffect(() => {
    api.get('/settings/public/cms')
      .then(res => {
        if (res.data?.data?.cancellationPolicy) {
          setContent(prev => ({
            ...prev,
            ...res.data.data.cancellationPolicy
          }))
        }
      })
      .catch(() => {})
          ...
          <div className="bg-[#f5b23e]/10 border-l-4 border-[#f5b23e] rounded-r-xl p-5 shadow-sm">
            <div className="flex gap-3">
              ...
              <p className="text-[13.5px] md:text-[14.5px] font-semibold text-[#d48806] leading-relaxed">
                <strong>Quick Tip:</strong> {content.warning}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6">
          {(content.steps || []).map((st, idx) => (
            <div key={idx} className="bg-white border border-gray-100 border-l-4 border-l-[#f5b23e] rounded-2xl p-6 shadow-sm space-y-3 hover:shadow-md transition-shadow">
              ...
              <p className="text-[14px] md:text-[15px] text-gray-600 font-medium leading-relaxed text-left md:text-justify">{st.desc}</p>
            </div>
          ))}
        </div>
